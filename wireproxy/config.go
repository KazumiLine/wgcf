...
// ParseConfig takes the prof struct and parses it into Configuration
func ParseConfigRaw(prof *wireguard.Profile) (*Configuration, error) {
	iniOpt := ini.LoadOptions{
		Insensitive:            true,
		AllowShadows:           true,
		AllowNonUniqueSections: true,
	}
	profData, err := wireguard.GenerateProfile(prof)
	if err != nil {
		return nil, err
	}
	cfg, err := ini.LoadSources(iniOpt, []byte(profData))
	if err != nil {
		return nil, err
	}
	device := &DeviceConfig{
